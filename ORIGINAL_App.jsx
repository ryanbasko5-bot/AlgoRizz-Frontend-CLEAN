...
export default function App() {
  ...
  const aeoStyles = `
    ...
    /* FIX: Inline bolding within paragraph */
    .aeo-answer-box p {
      margin: 0;
      font-weight: 500;
      font-size: 1.1rem;
    ...
    .aeo-answer-box strong {
      color: ${readableHeaderColor};
      font-weight: 700;
      display: inline; /* Ensure it doesn't break line */
    }
  ...
  const handleOptimize = async () => {
    ...
    const systemPrompt = `
      ...
      **You MUST preserve these links in the output.**
      If you rewrite a sentence that contained a link, attach the <a href> tag to the relevant keywords in the new sentence automatically.
      ...
      **You must strictly use British English spelling (e.g. colour, optimise, centre, programme, analyse) throughout the entire output.**
      ...
      1. **The Inverted Pyramid (Snippet Lock):** The first H2 must be a question answered immediately by the Key Takeaway Box.
      2. **Structure & Readability:** Avoid walls of text. Break content into short paragraphs (max 3 sentences) and use bullet points extensively for features, steps, or benefits.
      3. **Entity Density:** Bold key terms and entities related to the topic. Use HTML <strong> tags for bolding.
            ...
            <div className="lg:col-span-8 bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-[800px]">
                ...
                <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50">
                    <div className="flex gap-2 bg-slate-200 p-1 rounded-lg">
                        <button onClick={() => setActiveTab('preview')} className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === 'preview' ? 'bg-white shadow-sm' : 'text-slate-600 hover:text-slate-900'}`} style={activeTab === 'preview' ? { color: algorizzAccentColor } : {}}><Globe className="h-4 w-4" /> Preview</button>
                        <button onClick={() => setActiveTab('code')} className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === 'code' ? 'bg-white shadow-sm' : 'text-slate-600 hover:text-slate-900'}`} style={activeTab === 'code' ? { color: algorizzAccentColor } : {}}><Code className="h-4 w-4" /> HTML Code</button>
                    </div>
